    ...
    if request.method == 'POST' and request.is_ajax():
        ...
        if u_form.is_valid():
            ...
            if chart_type == 'MULTI-LINE-CHART':
                ...
                plot_url = my_functions.multiline_chart(current_modded_rows,
                                        current_dtypes_values, x_name, y_names)                  
            
            elif chart_type == 'LINE-CHART':
                plot_url = my_functions.single_line_chart(current_modded_rows, x_name, y_names, 
                                                current_dtypes_values)

            elif chart_type == 'BAR-CHART':
                plot_url = my_functions.bar_chart(current_modded_rows, x_name, y_names)
                pass
                
            elif chart_type == 'HISTOGRAM':
                plot_url = my_functions.histogram(current_modded_rows, x_name, y_names, 
                                       current_dtypes_values)
            elif chart_type == 'SCATTERPLOT':
                plot_url = my_functions.single_scatter_plot(current_modded_rows, x_name, y_names, 
                                       current_dtypes_values, True)

            elif chart_type == 'PIE-CHART':
                plot_url = my_functions.pie_chart(current_modded_rows, x_name, y_names, 
                                       current_dtypes_values)
...
def generate_plot_get(request):
    if request.is_ajax and request.method == "GET":
                generated_plot_url = request.GET.get("generated_plot_url", None)

                return JsonResponse({"generated_plot_url":generated_plot_url,
                 'url_redirect': '/project1/viz'}, status=200)
            
    return JsonResponse({"success":False,  'url_redirect': '/project1/viz'}, status=400)
